...
class AyahTrackerPresenter @Inject constructor(
  ...
  private fun handleAyahSelection(
    ...
    Timber.tag("handleAyahSelection").d("Long Press${result.toString()}")
    if (result != null) {
      if (eventType == SINGLE_TAP) {
        Timber.tag("handleAyahSelection").d(
          "SINGLE_TAP${
            AyahSelection.Ayah(
              result,
              getToolBarPosition(result.sura, result.ayah)
            )
          }"
        )
        Timber.tag("handleAyahSelection")
          .d("Toolbar${getToolBarPosition(result.sura, result.ayah)}")
        readingEventPresenter.onAyahSelection(
          AyahSelection.Ayah(result, getToolBarPosition(result.sura, result.ayah))
        )
      } else if (eventType == LONG_PRESS) {
        Timber.tag("handleAyahSelection").d("Long Press")
  ...
  private fun handleLongPress(selectedSuraAyah: SuraAyah) {
    ...
    Timber.tag("handleLongPress").d("Long Press${updatedAyahSelection}")
  ...
  override fun getSelectionBoundsForWord(
    ...
    val item = items.firstOrNull { it.page == page } ?: return null
    val selection =
      item.getToolBarPosition(item.page, word) as? SelectionIndicator.SelectedItemPosition
        ?: return null
    return selection
  }
